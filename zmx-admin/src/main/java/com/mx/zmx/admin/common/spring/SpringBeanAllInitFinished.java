package com.mx.zmx.admin.common.spring;

import com.mx.zmx.core.cache.redis.RedisUtil;
import com.mx.zmx.core.common.constant.SystemOperateEnum;
import com.mx.zmx.core.utils.OpenBrowserUtil;
import com.mx.zmx.core.utils.SystemOperateUtil;
    ...
    public void onApplicationEvent(ApplicationReadyEvent contextRefreshedEvent) {
       ...
       try {
        ...
        if(autoStartExplorer){
            String zmxUrl = String.format("http://localhost:%s%s",port,contextPath);
            log.info("正在使用浏览器打开管理后台地址："+zmxUrl);
            OpenBrowserUtil.openURL(explorerPath,zmxUrl);
        }
    }
}
